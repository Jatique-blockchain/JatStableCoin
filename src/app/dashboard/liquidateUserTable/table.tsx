...
import LiquidateUser from './liquidate';

const BorrowDetailsTable = ({ borrowerAddress }: { borrowerAddress: string }) => {
    ...
    if (userAddress === borrowerAddress) {
        return <CustomMessageCard message="You cannot liquidate yourself" />;
    }
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="hidden md:table-cell w-1/6">Borrow Time</TableHead>
                                    <TableHead className="w-1/2 md:w-1/6">Total Debt with Interest</TableHead>
                                    ...
                                    <Credenza key={index} open={openCredenza === 'repayBorrow'} onOpenChange={(open) => setOpenCredenza(open ? 'repayBorrow' : null)}>
                                        <CredenzaTrigger asChild>
                                            <TableRow onClick={() => handleCredenzaOpen(item)} className='cursor-pointer'>
                                                ...
                                                <TableCell className="text-xs md:text-base">{formatToDollars(formatUnits(item.totalDebtWithInterest, 18))}</TableCell>
